<template>
  <div class="image-model-management">
    <div class="actions-bar">
      <button @click="showAddModal = true" class="add-btn">+ 添加图片模型</button>
    </div>

    <div v-if="models.length === 0" class="empty-state">暂无图片模型</div>
    ...
    <div v-if="showAddModal || editingModel" class="modal-overlay" @click="closeModal">
      <div class="modal" @click.stop>
        <h3>{{ editingModel ? '编辑图片模型' : '添加图片模型' }}</h3>
...
import { imageModelDb } from '../../../utils/db';
...
const editingModel = ref<ImageModel | null>(null);
const modelForm = ref({
  modelName: '',
  modelPath: '',
  apiKey: '',
...
async function loadModels() {
  try {
    models.value = await imageModelDb.getByProjectId(props.projectId);
  } catch (err) {
    alert('加载模型失败: ' + (err as Error).message);
  }
}

function editModel(model: ImageModel) {
...
async function setDefault(modelId: number) {
  try {
    await imageModelDb.setDefault(props.projectId, modelId);
    await loadModels();
  } catch (err) {
...
async function deleteModel(modelId: number) {
  ...
  try {
    await imageModelDb.delete(modelId);
    await loadModels();
  } catch (err) {
