<template>
  <div class="video-model-management">
    <div class="actions-bar">
      <button @click="showAddModal = true" class="add-btn">+ 添加视频模型</button>
    </div>

    <div v-if="models.length === 0" class="empty-state">暂无视频模型</div>
    ...
    <div v-if="showAddModal || editingModel" class="modal-overlay" @click="closeModal">
      <div class="modal" @click.stop>
        <h3>{{ editingModel ? '编辑视频模型' : '添加视频模型' }}</h3>
...
import { videoModelDb } from '../../../utils/db';
...
const editingModel = ref<VideoModel | null>(null);
const modelForm = ref({
  modelName: '',
  modelPath: '',
  apiKey: '',
...
async function loadModels() {
  try {
    models.value = await videoModelDb.getByProjectId(props.projectId);
  } catch (err) {
    alert('加载模型失败: ' + (err as Error).message);
  }
}

function editModel(model: VideoModel) {
...
async function setDefault(modelId: number) {
  try {
    await videoModelDb.setDefault(props.projectId, modelId);
    await loadModels();
  } catch (err) {
...
async function deleteModel(modelId: number) {
  ...
  try {
    await videoModelDb.delete(modelId);
    await loadModels();
  } catch (err) {
